...
export function invalidate(...cacheKeys) {
  ...
  // Let any other currently-mounted module know these keys just went
  // stale, so it can refresh itself immediately instead of waiting for
  // the person to navigate back to it. See js/data-bus.js.
  if (cacheKeys.length) {
    window.dispatchEvent(new CustomEvent('pc:cache-invalidated', { detail: { keys: cacheKeys } }));
  }
}
